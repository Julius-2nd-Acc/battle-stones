...
def main():
    ...
    agent = EpsilonGreedyQLearningAgent(
    ...
    out = os.path.join(os.path.dirname(__file__), "..", "q_table_qlearn.pkl")
    if os.path.exists(out):
        try:
            with open(out, "rb") as fh:
                loaded_q = pickle.load(fh)
            # Try common ways to set the loaded Q-table on the agent
    ...
    num_episodes = 1000
    print("Training Q-learning for", num_episodes, "episodes...")
    agent.train(env, num_episodes=num_episodes, max_steps_per_episode=1000, verbose=True)

    with open(out, "wb") as fh:
        pickle.dump(agent.get_Q_table(), fh)
